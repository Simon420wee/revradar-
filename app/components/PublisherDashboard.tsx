...
import {
  DailyEntry, AdUnitEntry,
  getPublisherData, getPublisherAdUnits, getPublisherMessage, STORE_EVENT,
} from "../lib/store";
...
function NoData({ label = "Podaci još nisu dostupni za ovaj period." }: { label?: string }) {
  return (
    <div style={{ ...card, textAlign: "center", padding: "56px 0", color: "#4b5563" }}>
...
export default function PublisherDashboard({ publisherName, onLogout }: { publisherName: string; onLogout: () => void }) {
  ...
  const [adUnits, setAdUnits] = useState<AdUnitEntry[]>([]);
  const [message, setMessage] = useState("");
  const [ready, setReady] = useState(false);
  const [pdfBusy, setPdfBusy] = useState<string | null>(null);

  // Učitaj iz localStorage + osluškuj promene (admin sačuvao → odmah vidljivo)
  useEffect(() => {
    const load = () => {
      setData(getPublisherData(publisherName));
      setAdUnits(getPublisherAdUnits(publisherName));
      setMessage(getPublisherMessage(publisherName));
      setReady(true);
    };
    load();
    const onChange = () => load();
  ...
  const sortedDates = useMemo(() => data.map(r => r.date), [data]);
  const lastDate = sortedDates[sortedDates.length - 1];
  const lastDateLabel = lastDate ? lastDate.split("-").reverse().join(".") + "." : "—";

  const last30 = useMemo(() => data.slice(-30), [data]);
  const last7Sorted = useMemo(() => [...data].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 7), [data]);

  const today = data[data.length - 1] ?? null;
  const yesterday = data[data.length - 2] ?? null;
  ...
  const revenueChart = useMemo(() => last30.map(d => ({ ...d, dateLabel: d.date.slice(5).replace("-", "/") })), [last30]);
  ...
  return (
    <div style={{ position: "relative", display: "flex", fontFamily: "'Inter',system-ui,sans-serif", color: "#e2e8f0", minHeight: "100vh", background: AURORA_BG }}>
      ...
      <div style={{ flex: 1, minHeight: "100vh", padding: "32px 36px", overflowY: "auto", position: "relative", zIndex: 1 }}>
        {/* Poruka od admina — banner na vrhu */}
        {message.trim() && (
          <motion.div
            initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.25)", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
            <span style={{ fontSize: 16 }}>✉</span>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#fbbf24", letterSpacing: 0.6, marginBottom: 4 }}>OBAVEŠTENJE OD ADMINA</div>
              <div style={{ fontSize: 13, color: "#fde68a", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{message}</div>
            </div>
          </motion.div>
        )}
            ...
            {!hasData || !today ? <NoData /> : (
              <>
                ...
                <div style={{ ...card, marginBottom: 24 }}>
                  <div style={sectionTitle}>Prihod — poslednjih {Math.min(30, last30.length)} dana</div>
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={revenueChart}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                      <XAxis dataKey="dateLabel" tick={{ fill: "#4b5563", fontSize: 10 }} interval="preserveStartEnd" />
        ...
        {page === "adunits" && (
          <>
            <div style={{ marginBottom: 28 }}>
              ...
              <p style={{ color: "#4b5563", fontSize: 13, margin: "4px 0 0" }}>Performanse po formatu</p>
            </div>
            {adUnits.length === 0 ? <NoData label="Podaci o ad units još nisu dostupni." /> : (
              <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        ...
        {page === "reports" && (
          <>
            ...
            {months.length === 0 ? <NoData label="Nema podataka za izveštaje." /> : (
